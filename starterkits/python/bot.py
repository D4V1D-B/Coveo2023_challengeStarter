...
class Bot:
    ...
    def get_next_move(self, game_message: GameMessage):
        ...
        listePriorite = sorted(game_message.meteors, key=lambda meteor: meteor.meteorType, reverse=True)
        ...
        # print(a)
        Dx = game_message.cannon.position.x - listePriorite[0].position.x
        Dy = game_message.cannon.position.y - listePriorite[0].position.y
        b = 2 * (Dx * listePriorite[0].velocity.x + Dy *
                 listePriorite[0].velocity.y)
        # print(b)
        c = -(((game_message.cannon.position.x - listePriorite[0].position.x) ** 2 + (
                    game_message.cannon.position.y - listePriorite[0].position.y) ** 2) ** 0.5) ** 2
        # print(c)
        d = (b ** 2) - (4 * a * c)
        if d < 0:
            return []
